...
st.write("Supervised machine learning is a type of machine learning where the model is trained on a labeled dataset, meaning that the input data is paired with the correct output labels. The goal is to learn a mapping from inputs to outputs so that the model can make predictions on new, unseen data.")
...
if technique_options == "Linear Regression":
    ...
    if target_variables in feature_variables:
        ...
        colored_box("Feedback: R squared is a measure of how well the model explains the variance in the target variable. A higher R squared value indicates a better fit of the model to the data. RMSE is a measure of the average distance between the predicted values and the actual values. A lower RMSE value indicates better performance of the model.", color="#34d16b")
...
# Decision Tree code section
    ...
    if target_variable in feature_variables:
        ...
        if st.button("Run Your Decision Tree Analysis"):
            ...
            colored_box("Guide: Accuracy scores record the portion of all predictions that our model correctly predicted. Precision scores tell us the portion of positives our model correctly predicted.", color = "#f0a9ba")
            if y.unique().shape[0] == 2:
                y_prob = model.predict_proba(X_test)[:, 1]
                roc_auc = roc_auc_score(y_test, y_prob)
                
            #else: 
                #st.write("ROC AUC Score is not applicable for multi-class classification. It is only applied to binary target variables.")
            ...
            st.pyplot(fig_cm)
            colored_box("Guide: The Confusion Matrix gives us a visualization of where the model is predicting correctly, and where it is predicting incorrectly. In the top left, we can see how many negatives were correctly identified. In the bottom right, we can see how many positives were correctly identified", color = "#f0a9ba")
            ...
            if y.nunique() == 2:
                ...
                colored_box("Guide: The ROC Curve displays the tradeoff between true positives and true negatives. When the curve is farther away from the straight line, it is associated with better model performance.", color = "#f0a9ba")
            ...
            colored_box("Guide: The decision tree visualization tells us which feature variables were used to split the data.", color = "#f0a9ba")
            plt.clf()  # Clear the figure to avoid overlap with next plot
